...
-- todo: replace returns with transformations of existing vector
------------------------------
...
function vector:mult(vm)
  local x = type(vm) == "table" and vm.x or vm
  local y = type(vm) == "table" and vm.y or vm
  self.x = self.x * x
  self.y = self.y * y
end
...
function vector:heading()
  return -1*math.atan(self.y, self.x)
end
...
function vector:rotate(angle)
  ...
  fa = self.from_angle(self:heading() + angle)
